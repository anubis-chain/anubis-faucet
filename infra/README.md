...
For an eligible AWS **Paid account plan**, subscribe each deployed distribution
to the CloudFront `FREE` flat-rate plan. It includes the associated CloudFront
distribution and WAF web ACL at $0/month for the plan allowance. AWS requires
exactly one distribution ARN and one dedicated WAF ARN in each subscription;
this CDK app creates that one-to-one layout and outputs both values.
...
Confirm that the returned status becomes `ACTIVE`. Accounts that selected the
AWS Free account plan are not eligible; keep pay-as-you-go pricing in that case.
Do not create a paid `PRO`, `BUSINESS`, or `PREMIUM` subscription without a
separate human billing approval.
...
lowercase hexadecimal digest in `x-amz-content-sha256`. The distribution forwards
that header and `CloudFront-Viewer-Address`; requests without a matching digest
fail before the Lambda handler with `InvalidSignatureException`.
...
After the incident, first deploy with `-c faucetEnabled=false`, restore the API
reserved concurrency to `5`, confirm reconciliation is healthy, and only enable
new claims after another preflight.
